...
- Notification Shade Controls:
  ...
  - Starting a nap is disabled on `MainActivity` and unavailable in notifications while a sleep session is in progress (`now > currentAlarmTime - (minSleepDuration * 1.2)`).
  - The secondary action displays **I'm Awake** during the pre-alarm window (`now > currentAlarmTime - (minSleepDuration * 1.2) && now < currentAlarmTime`), during active naps, or while alarms are ringing/snoozed (serving as the alarm dismiss action). When no sleep session is in progress (Idle Phase), it displays **Nap**. During active sleep sessions prior to the pre-alarm window, the Nap action is omitted.
