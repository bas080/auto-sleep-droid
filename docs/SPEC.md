...
- **How It Works**:
  ...
  6. **Wake-Up Alarm Gestures & Persistence**:
     ...
     - **Early Wake-Up ("I'm Awake")**: When waking up before the alarm and launching the app during an active sleep session (`sleep_start_time_ms` within the last 14 hours), a cancelable confirmation dialog ("Are you awake?") is automatically presented on launch, as well as via the ongoing status notification action (replacing Nap when wake alarm is enabled). Confirming registers a sleep session in Health Connect (if enabled), cancels today's pending wake alarm, steps the wake-up time 15 minutes back towards the target goal time, and reschedules the alarm for tomorrow.
...
- **Nap Alarm & Reset Behavior**:
  ...
  - When the sleep timer is reset (via flip gesture, volume button press, or duration update), an active nap alarm is pushed forward by the same reset increment. Dismissing a nap alarm when triggered does not affect or adjust the current wake-up time.
