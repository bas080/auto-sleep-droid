...
- Notification Shade Controls:
  ...
  - The secondary action displays **I'm Awake** during the pre-alarm window (`now >= currentWakeTime - (minSleepDuration * 0.5) && now < currentWakeTime`), during active naps, or when an ongoing session reaches or passes `currentWakeTime` (`isSessionOngoing && now >= currentWakeTime`, serving as the alarm dismiss action; snoozing advances `currentWakeTime`). When no sleep session is in progress (Idle Phase), it displays **Nap**. During active sleep sessions prior to the pre-alarm window, the Nap action is omitted.
