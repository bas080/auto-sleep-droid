...
- **How It Works**:
  ...
  6. **Wake-Up Alarm Gestures & Persistence**:
     ...
     - **Early Wake-Up ("I'm Awake")**: When waking up before the alarm during an active sleep session (`sleep_start_time_ms` within the last 14 hours or active sleep timer), the ongoing status notification replaces the Nap action with **I'm Awake**. Confirming **I'm Awake** (or answering the automatic prompt on launch) registers a sleep session in Health Connect (if enabled), turns off active sleep timers, clears `sleep_start_time_ms`, cancels today's pending wake alarm, steps the wake-up time 15 minutes back towards the target goal time, reschedules the alarm for tomorrow, and reverts the notification action back to **Nap**.
