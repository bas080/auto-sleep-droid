...
- Notification Shade Controls:
  ...
  - Starting a nap is disabled on `MainActivity` and unavailable in notifications while a sleep session is in progress (`now >= currentWakeTime - (minSleepDuration * 1.2) && now < currentWakeTime`).
  - The secondary action displays **I'm Awake** during the pre-alarm window (`now >= currentWakeTime - (minSleepDuration * 0.5) && now < currentWakeTime`), during active naps, or when an ongoing session passes `currentWakeTime` or alarms are ringing/snoozed (`(isSessionOngoing && now >= currentWakeTime) || isAlarmRinging || isAlarmSnoozed`, serving as the alarm dismiss action). When no sleep session is in progress (Idle Phase), it displays **Nap**. During active sleep sessions prior to the pre-alarm window, the Nap action is omitted.
