...
Releases are generated via GitHub Actions (`.github/workflows/android-release.yml`):
...
3. **Pull Request Push:** Pull request pushes trigger the `build-test-release` job to run unit tests, build the release APK, and publish a draft release (`pr-<number>`, marked `--draft=true`) that is not visible to the public.
