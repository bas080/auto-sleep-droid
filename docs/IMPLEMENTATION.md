...
- Invoke `startForeground` with `ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK` on Android 10+ (API 29+), wrapping calls in `showOrHideNotification()` inside a `try-catch` block to handle `ForegroundServiceStartNotAllowedException` (Android 12+) or `IllegalStateException` gracefully when starting from background without foreground exemptions, logging failures to `EventLogger` and falling back to `NotificationManager.notify()` while preserving internal service state and recovery on next UI launch.
- Re-invoke `startTimerService()` in `MainActivity.onResume()` to provide self-healing recovery that promotes `MainService` back to a foreground service as soon as the app enters the foreground.
- Safely wrap calls to `startForegroundService()` in `MainActivity`, `BootReceiver`, and `AwakeDialogActivity` in `try-catch` blocks logging failure context to `EventLogger`.
...
- Crash Reporting & Feedback Emails: `sendFeedbackEmail` constructs structured email templates targeting `bas080@hotmail.com` via `ACTION_SENDTO`. For crash reports, the email subject is "Auto Sleep Droid Crash Report" and includes troubleshooting questions (prior user actions, frequency, active features), exception details, optional logs, and app/device metadata. For general feedback, the subject is "Auto Sleep Droid Feedback" and includes feedback questions (target feature/aspect, description/desired improvements, bug reproduction steps), optional logs, and metadata.
...
Write code with the least amount of defensive null guards and `try-catch` blocks necessary. Code fails fast on unexpected state, caught by global exception handler in `AutoSleepApplication`. Defensive wrapping of operations in `try-catch` blocks is explicitly prohibited across the codebase.
...
Centralized append-only logging utility that formats event lines with timestamps and importance levels (`LEVEL_LOW`, `LEVEL_NORMAL`, `LEVEL_HIGH`). Persists logs directly to internal app storage (`event_logs.txt`) without keeping log lists in memory or enforcing a fixed line count. If appending to the log file fails, it prunes the file to the 50 most recent lines and retries. Supports clearing logs via `EventLogger.clear(context)`.
...
| `donate_dialog_hidden` | boolean | Whether random donate dialog popups are hidden in the future after user taps Donate |
...
1. **Manual Workflow Dispatch:** Trigger the GitHub Actions workflow (`.github/workflows/android-release.yml`) manually using `workflow_dispatch` and supply the target version (e.g. `0.2.4`). The `trigger-release` job executes `./scripts/release.sh "${VERSION_NAME}"` to bump `versionCode` and `versionName` in `app/build.gradle`, creates a Git commit and version tag (`v${VERSION_NAME}`), and pushes `master` and the tag to GitHub. The `build-test-release` job then runs sequentially after `trigger-release`, checking out the newly created release tag to run unit tests, build the release APK, and publish the GitHub release.
