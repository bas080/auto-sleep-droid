...
3. If the change was not generated by the app and the timer is active, the service restarts the timer at `configuredDurationMinutes`. If a fade is in progress, `startTimer()` cancels the fade callbacks before any volume restore can occur, so the user's new volume is preserved.
