...
- **Foreground Service Exception Handling (`ForegroundServiceStartNotAllowedException`):**
  When analyzing crash reports involving `android.app.ForegroundServiceStartNotAllowedException` during `startForeground()`, understand that on Android 12+ (API 31+) calling `startForeground()` from the background without a foreground launch exemption (e.g. OS `START_STICKY` service restart) throws `ForegroundServiceStartNotAllowedException` (a subclass of `IllegalStateException`).
  - *Reasoning & Workability:* Wrapping `startForeground()` calls in `showOrHideNotification()` inside a `try-catch` block prevents process crashes. When caught, `MainService` safely logs the failure to `EventLogger` and continues operating as a background service with all timer states, alarms, audio focus handlers, and sleep tracking intact.
  - *Recovery:* When the user next opens `MainActivity`, `startForegroundService()` is launched from the foreground, granting the exemption and restoring the ongoing status notification seamlessly.
  - *Foreground Service Type:* On API 29+, pass `ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK` to `startForeground()` to align with manifest declarations.
