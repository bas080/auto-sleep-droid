...
- **Minimal Null Guards & Fail-Fast Error Handling:** Write code with the least amount of null guards and `try-catch` blocks necessary. Never use `try-catch` blocks to swallow errors or exceptions that result in a potentially degraded state of the app. Allow exceptions to fail fast and surface so that the global uncaught exception handler (`AutoSleepApplication`) can intercept the error, record diagnostic context/stack traces, and prompt the user appropriately rather than silently swallowing errors or continuing in a corrupted or degraded state.
