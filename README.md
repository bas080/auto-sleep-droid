...
- **Screen-free resets:** Simply press a physical volume button while in bed to reset the sleep timer without looking at a bright screen.
...
3. **Notification controls:** Control ongoing timer state with **Disable** / **Enable** actions directly in the notification shade.
...
5. **Smart wake-up alarm:** If enabled, the app automatically calculates and sets your daily wake-up alarm for your target time (or later if you went to bed late) to protect your minimum sleep duration. Press a volume button to snooze for 9 minutes, or tap **I'm Awake** to stop the alarm.
...
4. If you're still awake, press your phone's volume buttons at any time to reset the timer to full duration.
5. Tap **Disable** or **Enable** in the notification shade to toggle the sleep timer at any time.
6. Access action links at the bottom of the main screen (**Manual**, **Logs**, **Feedback**, **Donate**, **Export**, and **Import**) arranged inline with middle dots.
...
4. When the alarm rings or is snoozed, press any physical volume button to snooze for 9 minutes. Tapping **I'm Awake** on the notification or main UI stops the alarm.
...
- **Vibration (`VIBRATE`)** *(Optional)*: Provides faint haptic feedback confirming your actions (turning off/on and volume button resets).
...
- **Health Connect Write Sleep (`android.permission.health.WRITE_SLEEP`)** *(Optional)*: Persists sleep and wake times as sleep sessions to Android Health Connect when Health Connect synchronization is enabled.
- **Do Not Disturb Access (`android.permission.ACCESS_NOTIFICATION_POLICY`)** *(Optional)*: Checks DND state when Auto Sleep Timer (DND) mode is enabled.
