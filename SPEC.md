...
## Product goal
Provide an Android sleep timer controlled via a Quick Settings tile toggle button in the pull-down drawer and notification controls. The main UI displays a live event log (one line per event) for debugging.

## System states
- Off: The timer is manually disabled. Media continues playing normally, no notification is shown, and the current volume remains entirely unchanged.
...
- Off:
  - No notification is displayed when auto-sleep is turned off.
- Waiting: 
...
## User interface
- Provide a Quick Settings tile toggle button in the system pull-down drawer to turn auto-sleep on or off.
- Display a line-by-line list of timestamped events in the main UI for debugging purposes.
- Show a notification only when auto-sleep is turned on (in Waiting, Active, or Fading states).
- Use notification buttons for sleep-timer controls ("Set Timer" inline reply to change duration and "Turn Off" action button).
- Tapping/clicking the notification body expands or collapses the notification in all visible states.
- Keep the notification ongoing while auto-sleep is on so the user cannot swipe it away or accidentally dismiss it.
- Enter the duration through a minimal inline notification reply using Android's native text input mechanism.

## Timer configuration
- The user can turn the sleep timer on or off using the Quick Settings tile toggle in the pull-down drawer or the "Turn Off" action in the notification.
...
- When the timer is turned off: remove the notification, leave the current volume unchanged, and allow media to continue playing.
